...
#include "BVtabulated6DoFMotion.H"
...
{
    defineTypeNameAndDebug(BVtabulated6DoFMotion, 0);
...
Foam::solidBodyMotionFunctions::BVtabulated6DoFMotion::BVtabulated6DoFMotion
(
...
Foam::septernion
Foam::solidBodyMotionFunctions::BVtabulated6DoFMotion::transformation() const
{
    ...
    if (t < times_[0])
    {
        FatalErrorIn
        (
            "solidBodyMotionFunctions::BVtabulated6DoFMotion::transformation()"
        )   << "current time (" << t
    ...
    quaternion R = quaternion(vector(0, 0, 1), TRV[1].z());
    R *= quaternion(vector(0, 1, 0), TRV[1].y());
    R *= quaternion(vector(1, 0, 0), TRV[1].x());

    septernion TR(septernion(CofG_ + TRV[0])*R*septernion(-CofG_));

    Info<< "solidBodyMotionFunctions::BVtabulated6DoFMotion::transformation(): "
...
bool Foam::solidBodyMotionFunctions::BVtabulated6DoFMotion::read
(
    const dictionary& SBMFCoeffs
)
{
    ...
    if (newTimeDataFileName != timeDataFileName_)
    {
        ...
        else
        {
            FatalErrorIn
            (
                "solidBodyMotionFunctions::BVtabulated6DoFMotion::read"
                "(const dictionary&)"
