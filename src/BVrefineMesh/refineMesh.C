...
int main(int argc, char *argv[])
{
    ...
    (
        "noCellLevel",
        "do not update cellLevel (snappyHexMesh will fail if cellLevel is kept.)"
    );
    ...
    (
        "refineUpToLevel",
        "int"
        "Refine up to a specified level"
    );
    ...
    const bool noCellLevel = args.optionFound("noCellLevel");
...
//        }
    ...
    /* sose: refinement level */
    label refineUptoLevel(refineDict.lookupOrDefault<label>("refineUptoCellLevel",-1));
    args.optionReadIfPresent("refineUpToLevel", refineUptoLevel);
    ...
    //if (refineUptoLevel != 0)
    {
        Info << "Refine cells up to level: " << refineUptoLevel << endl;
        cellSet newCells(mesh, "refinedCells", refCells.size());
        label count(0);        
        forAll(refCells, i)
        {
            label cellI(refCells[i]);
            if ((cellLevel[cellI] < refineUptoLevel) || (refineUptoLevel < 0))
            {
        ...
        refCells.clear();
        refCells = newCells.toc();

        //Info << "debug: " << cellLevel.size() << endl;
        //Info << "debug: " << max(cellLevel) << endl;
        //Info << "debug: " << refCells.size() << endl;
        //Info << "debug: " << count << endl;        
        //return 0;
    }
    ...
    if (!noCellLevel)
    {
        Info << "Update cellLevel ... " << nl << endl;
        labelIOList newCellLevel
        (
            IOobject
            (
                "cellLevel",
        ...
        forAll(newToOld, i)
        {
            label oldCellId(newToOld[i]);
            newCellLevel[i] = cellLevel[oldCellId];
        }
        newCellLevel.write();
    }
