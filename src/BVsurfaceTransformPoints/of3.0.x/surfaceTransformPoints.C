...
int main(int argc, char *argv[])
{
    ...
    (
        "EulerZYX",
        "vector",
        "Apply rotation in terms of '( roll pitch yaw )' in degrees"
    );
    argList::addOption
    (
        "CoR",
        "vector",
        "define center-of-rotation (for EulerZYX only)"
    );
    ...
    else if (args.optionReadIfPresent("EulerZYX", v))
    {
        vector CoR(0,0,0); args.optionReadIfPresent("CoR", CoR);
        Info<< "EulerZYX: Rotating points by (deg)" << nl
            << "    roll  " << v.x() << nl
            << "    pitch " << v.y() << nl
            << "    yaw   " << v.z() << nl;
        Info<< "EulerZYX: Center-of-rotation: " << CoR << endl;

        // Convert to radians
        v *= pi/180.0;

        quaternion R = quaternion(vector(0, 0, 1), v.z());
        R *= quaternion(vector(0, 1, 0), v.y());
        R *= quaternion(vector(1, 0, 0), v.x());
