...
class BoundaryK(ReadWriteFile):
    ...
    def Build(cls,  case, symmetry=1, wallFunction=False, application="foamStar", namePatch=namePatch, k=0.00015, case2D=False):
        ...
        if wallFunction:
            res["boundaryField"][patch["structure"]] = {"type": "kqRWallFunction"}
...
class BoundaryAlpha(ReadWriteFile):
    ...
    """
    @classmethod
    def Build(cls, case, symmetry=1, namePatch=namePatch, case2D=False, wave=True, relaxZone=True, struct='', application="foamStar"):

        patch = namePatch[application]
        if application == "foamStar":
            res = cls(name=join(case, getFilePath("boundaryAlpha")), read=False)
        else:
            res = cls(name=join(case, "0", "org", alpha[application]), read=False)
        res.header["class"] = "volScalarField"
        ...
        bf = DictProxy()
        if wave:
            wavePatch = {"type": waveAlpha[application], "value": "uniform 0"}
        else:
            wavePatch = {"type": "zeroGradient"}
...
class BoundaryVelocity(ReadWriteFile):
    ...
    """
    @classmethod
    def Build(cls, case, speed=0., symmetry=1, namePatch=namePatch, case2D=False, wave=True, relaxZone=True, struct='', application="foamStar"):
        ...
        if wave:
            wavePatch = {"type": waveVelocity[application], "value": "uniform (0 0 0)"}
        else:
            wavePatch = {"type": "fixedValue", "value": "uniform (0. 0. 0.)"}
        ...
        if len(struct) > 0:
            bf[struct] = {"type": "movingWallVelocity", "value": "uniform (0 0 0)"}
        else:
